...
public class Quiz {
	...
	boolean answer24 =true; // last_name is located under the WHERE location and will run, however the first_name will not run because it has no     
						    // WHERE location, so that part will error.
	...
	boolean answer27 =false; // However, alternative words are used: Read/Retrieve, https://en.wikipedia.org/wiki/Create,_read,_update_and_delete 
						    //therefore retrieve would be considered Read(true) But per your SQL Select video, it's Create, Read, Update, and Delete.
	...
	boolean answer31 =false; //Correct method: DROP Table table_name;
	
	// 32. `DELETE DATABASE [database name];` deletes a database;
	boolean answer32 =false; //Correct method: DROP DATABASE DatabaseName;
	
	// 33. ERD stand for Every Relationship Diagram.
	boolean answer33 =false; //Correct answer: Entity Relationship  Diagram. 
	
	// 34. `UPDATE COLUMN` is used to change a fields value.
	boolean answer34 =false; //Correct method: UPDATE (table name) SET
			
	// 35. `INSERT INTO [database name]` will create a new table.
	boolean answer35 =false; //Correct method: CREATE Table table_name(        ...continue next lines...   );
	 ...
	 *  SELECT t.title, count(e.emp_no) FROM employees e
    ->  INNER JOIN titles t
